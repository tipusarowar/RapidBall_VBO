...
class Grid {
	...
	public Grid(int vertsAcross, int vertsDown, boolean useFixedPoint) {
		...
		mIndexCount = indexCount; // 6
		mIndexBuffer = ByteBuffer.allocateDirect(CHAR_SIZE * indexCount) // 2*6=12
	...
	void set(int i, int j, float x, float y, float z, float u, float v, float[] color) {
		...
		final int index = mW * j + i; // mW=2 i=1 j=1 index=2*1+1=3

		final int posIndex = index * 3; // 3*3=9 
		final int texIndex = index * 2; // 3*2=6
		final int colorIndex = index * 4; // 3*4=12

		if (mCoordinateType == GL10.GL_FLOAT) {
			mFloatVertexBuffer.put(posIndex, x); // @ posind 9 put x or width
			mFloatVertexBuffer.put(posIndex + 1, y); // @ posind 9+1=10 put y or height
			mFloatVertexBuffer.put(posIndex + 2, z); // @ posind 9+2=11 put z or depth
